...
class ExposedDiffRepository : DiffRepository {
    override suspend fun createProject(name: String): ProjectResponse = DatabaseFactory.query {
    ...
    override suspend fun projectExists(projectId: UUID): Boolean = DatabaseFactory.query {
        ProjectsTable.selectAll().where { ProjectsTable.id eq projectId }.limit(1).any()
    }

    override suspend fun createAsset(projectId: UUID, stored: StorageService.StoredFile): AssetResponse = DatabaseFactory.query {
        val id = UUID.randomUUID()
        val now = Instant.now()
        AssetsTable.insert {
    ...
    override suspend fun getAsset(assetId: UUID): AssetResponse? = DatabaseFactory.query {
    ...
    override suspend fun createComparison(projectId: UUID, oldAssetId: UUID, newAssetId: UUID): ComparisonResponse? = DatabaseFactory.query {
        val oldExists = AssetsTable.selectAll().where {
            (AssetsTable.id eq oldAssetId) and (AssetsTable.projectId eq projectId)
        }.limit(1).any()
        val newExists = AssetsTable.selectAll().where {
    ...
    override suspend fun getComparison(comparisonId: UUID): ComparisonResponse? = DatabaseFactory.query {
    ...
    override suspend fun createRun(runId: UUID, comparisonId: UUID, outputDir: String): RunResponse = DatabaseFactory.query {
        val now = Instant.now()
        RunsTable.insert {
    ...
    override suspend fun getRun(runId: UUID): RunResponse? = DatabaseFactory.query {
        RunsTable.selectAll()
    ...
    override suspend fun listArtifacts(runId: UUID): List<ArtifactResponse> = DatabaseFactory.query {
    ...
    override suspend fun getArtifact(runId: UUID, artifactId: UUID): ArtifactResponse? = DatabaseFactory.query {
        ArtifactsTable.selectAll()
    ...
    override suspend fun getReportArtifact(runId: UUID): ArtifactResponse? = DatabaseFactory.query {
        ArtifactsTable.selectAll()
            .where { (ArtifactsTable.runId eq runId) and (ArtifactsTable.kind eq ArtifactKind.REPORT_HTML.name) }
            .limit(1)
            .map { it.toArtifactResponse() }
            .singleOrNull()
    }

    override suspend fun claimNextQueuedRun(): QueuedRunWork? = DatabaseFactory.query {
        val nextRun = RunsTable.selectAll()
        ...
        val comparison = ComparisonsTable.selectAll()
            .where { ComparisonsTable.id eq comparisonId }
            .limit(1)
            .firstOrNull() ?: return@query null

        val oldAsset = AssetsTable.selectAll()
            .where { AssetsTable.id eq comparison[ComparisonsTable.oldAssetId] }
            .limit(1)
            .firstOrNull() ?: return@query null

        val newAsset = AssetsTable.selectAll()
            .where { AssetsTable.id eq comparison[ComparisonsTable.newAssetId] }
            .limit(1)
            .firstOrNull() ?: return@query null

        val updated = RunsTable.update({ (RunsTable.id eq runId) and (RunsTable.status eq RunStatus.QUEUED.name) }) {
