...
class RunWorkerTest {
    ...
    fun workerMarksRunSucceededAndRegistersArtifacts() = runBlocking {
        ...
        val outputDir =
            config.runsDir.resolve("run-1").createDirectories().toAbsolutePath().toString()
    ...
    @Test
    fun workerMarksRunFailedWhenRunnerThrows() = runBlocking {
        val dataRoot = createTempDirectory("worker-test-failure-data")
        val config =
            AppConfig(
                dbUrl = "jdbc:postgresql://unused",
                dbUser = "unused",
        ...
        val newFile = Files.createTempFile("new", ".pdf").toAbsolutePath().toString()
        val outputDir =
            config.runsDir.resolve("run-2").createDirectories().toAbsolutePath().toString()
        val runId = repository.enqueueRun(oldFile, newFile, outputDir)
        ...
        val runner =
            object : VisualDiffRunner {
                override fun run(
                    ...
                    throw IllegalStateException("visual diff crashed")
                }
            }

        val worker =
            RunWorker(
                logger = LoggerFactory.getLogger("RunWorkerFailureTest"),
                config = config,
                storage = storage,
                repository = repository,
                visualDiffRunner = runner,
        ...
        assertTrue(run.errorText?.contains("visual diff crashed") == true)
    }
}
